...
(`max-sixty/tend/claude@X.Y.Z` for Claude, `max-sixty/tend/codex@X.Y.Z` for
Codex; the older `max-sixty/tend@X.Y.Z` still resolves to the Claude action for
adopters pinned before the `/claude` subpath existed).
