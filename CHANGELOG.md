...
- **The CI poll's settle loop is bounded by a shared sleep budget**, so checks that keep registering can no longer stretch it past the command timeout. A command timeout counts as unverified. ([#1376](https://github.com/max-sixty/tend/pull/1376))
