...
- **The agent runs in the job's own checkout and home again, through a copy-on-write view**, as 0.2.12 introduced and 0.2.13 reverted. What `setup:` prepares — a toolchain, a warm cache, a generated file — is there for the agent at the path `setup:` left it, and nothing the agent writes reaches the runner's disk or a later step. The checkout no longer carries the sandbox's write-protection stubs that failed `git add -A` under 0.2.12, and the agent starts in a repository without `.claude/`. Anything `setup:` leaves readable in the runner's home, exports to `$GITHUB_ENV`, or sets in job-level `env:` is readable by the agent, and so by a pull request's own code: log in only in steps tend does not run. `sandbox_env` refuses `GITHUB_*` keys and `CLAUDE_CONFIG_DIR` at `init`. A self-hosted runner whose work folder is outside the runner account's home fails in the "Set up credential-isolation sandbox" step, naming both paths. ([#1346](https://github.com/max-sixty/tend/pull/1346))
- **`sandbox_setup:` and `sandbox_path:` are deprecated, and `init` migrates them.** It warns, and turns each key into one `setup:` step after the consumer's own: `sandbox_path` entries become `$GITHUB_PATH` lines that keep their order, and `sandbox_setup`'s commands share one bash step. A migrated command no longer has tend's `uv` on PATH, so one that calls `uv` needs `astral-sh/setup-uv` earlier in `setup:`. A later release will refuse both keys. ([#1346](https://github.com/max-sixty/tend/pull/1346))
