...
{setup_uv}
      - name: Verify generator output matches committed files
        ...
        run: |
          ...
          # `git diff` alone ignores untracked files, so a workflow the regen
          # newly emits but the PR never committed would pass green. The
          # intent-to-add brings it into the diff and stages the removal of
          # any workflow the regen deleted, so compare against HEAD to catch
          # both. The job never commits, so the index write is inert.
          git add --intent-to-add .github/workflows/
          if ! git diff HEAD --quiet .github/workflows/; then
            ...
            git --no-pager diff HEAD .github/workflows/
            exit 1
          fi
"""
