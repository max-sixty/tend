...
def _action_ref(cfg: Config) -> str:
    """Ref string for the harness-specific composite action, pinned to the
    generator's own release tag.

    Both actions ship from this repo; the Codex variant lives under /codex/.
    The generator package version and the git tag are the same commit (release:
    bump pyproject, merge, `git tag X.Y.Z` on that commit, then PyPI publish).
    Pinning the action to that tag gives each generated workflow exact
    provenance: the YAML says "generated by tend X.Y.Z" and runs `tend@X.Y.Z`,
    the identical commit. Release tags are immutable (tag ruleset; see
    docs/security-model.md), so the pin is a stable supply-chain anchor.
    Consumers move forward via the nightly `uvx tend@latest init` regen
    pulling a newer generator, which restamps a newer tag as a reviewable
    diff in their own repo.

    `version("tend")` only resolves to an existing tag when generation runs
    from a released package. Generating from in-tree source pins a tag that
    does not exist yet; never commit those workflows (see CLAUDE.md).
    """
    base = "max-sixty/tend" if cfg.harness == "claude" else "max-sixty/tend/codex"
    return f"{base}@{version('tend')}"
...
def _agent_step(
    ...
    *,
    ...
    """Render the `uses: max-sixty/tend@<release tag>` step.
