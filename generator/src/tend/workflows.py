        ...
        with:
          ...
          prompt: >-
            ...
            ${{{{ github.event_name == 'issues'
              ...
              || format('A user commented on an issue/PR where you previously participated ({{0}}). Read the full context. Only respond if the comment is directed at you, asks a question you can help with, or requests changes you can make. A comment that responds to concerns you raised in a review is directed at you — briefly acknowledge that the concerns are resolved (or explain why they are not). If the conversation is between humans, exit silently. In multi-way conversations (other humans besides you and the commenter are participating), apply a higher bar: only respond if you have something genuinely new to add — a code fix, reproduction, or specific technical detail no one else provided. Do not restate, agree with, or summarize what a maintainer said. When in doubt, exit silently.', github.event.comment.html_url)
            }}}}
