...
Tend's bundled skills provide defaults; the repo overrides them — through its `running-tend` skill, its `CLAUDE.md` or `AGENTS.md`, and its `.config/tend.yaml`. **Where they conflict, the repo wins**, across every skill, not just this one.
...
| Before writing a code fix for a bug | `references/fixing.md` | reproducing first, the bar for attempting a fix, shapes that are not a fix, fixing the bot's own behavior |
...
| After any push you are accountable for, or before calling a failure pre-existing | `references/ci-monitoring.md` | the pinned poll, the default-branch check a pre-existing claim needs, a review that lands mid-poll, rerunning failed jobs |
...
**Your own prior comment.** If you are responding to your own prior comment or review (not a human's reply to it), exit silently to avoid self-conversation loops. That includes a review your own review workflow left on your PR: the session that posts such a review applies the findings it raised, so nothing is waiting on a reply. A freshly-opened issue the bot authored with no prior bot comments (nightly failure, CI report, code-quality finding) is a report to act on, not a self-conversation: triage it normally. **Recheck before posting** in `references/posting.md` still prevents a duplicate triage comment if a sibling run fires on the same issue.
...
Read logs, code, and API data before drawing conclusions. Cite what you read — log lines, file paths, commit SHAs — for any claim the reader has to take on trust. Trace causation — if two things co-occur, find the mechanism rather than saying "this may be related." Distinguish what you verified from what you inferred.
