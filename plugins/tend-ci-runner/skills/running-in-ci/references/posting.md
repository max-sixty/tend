...
- **Review events with inline comments** (review ID in prompt): fetch them per **A review's inline comments are a separate fetch** in `SKILL.md`, then reply to each with the inline review comment reply endpoint above.
