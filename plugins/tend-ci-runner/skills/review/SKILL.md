...
- In the bot's own automation (its skills, and the scheduled workflows that invoke it): is the change worth its weight? Challenge new mechanism whose only payoff is saved compute — retries, skip-gates, caches, scheduling arithmetic — and ask for waste that has recurred across days, not projected waste. Judge the whole change: a PR well-argued line by line can still cost more machinery than the compute it saves, and the review should say so plainly; the same setting repeated across workflows, jobs, or call sites is a configuration scheme, not a knob. Recommend one knob in one place, or closing the PR, over refinement. In the rest of the repo's CI, outside tend's own workflows, this is the maintainers' call rather than a default to impose: raise the cost where you see it and let them weigh it.
