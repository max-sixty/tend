...
- **`tend-notifications` mark-reading a cross-repo `CheckSuite` `ci_activity` notification from an abandoned fork and exiting silently.** A fork that doesn't sync upstream keeps running its own scheduled `tend-*` workflows; when those fail, the canonical bot receives `reason: ci_activity` `CheckSuite` notifications about another repo's CI. The notifications skill classifies them as cross-repo not-actionable and marks them read, but the agent still spins up. PR [#392](https://github.com/max-sixty/tend/pull/392) ("feat(notifications): drop cross-repo CheckSuite notifications in pre-check") proposed a YAML pre-check filter and was closed by the maintainer as superseded by [#375](https://github.com/max-sixty/tend/pull/375)'s `github.repository_owner` fork-owner guard (released in 0.0.17): synced forks pick up the guard and no-op on the cron, and the residual cost of abandoned-fork notifications is "small enough not to justify a permanent pre-check layer." Do not re-propose a pre-check filter keyed on `reason`, `subject_type`, or sender, and do not maintain a per-month occurrence threshold for this notification shape — record as carry observations and move on.
