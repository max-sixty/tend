...
- **Merging**: Follow the system prompt's merge mode. Never enable auto-merge. Under `maintainer`, PRs are proposals and only a maintainer lands them. Under `yolo`, merge only through the pull-request API with the current head SHA, after the CI gate in `/tend-ci-runner:monitor-ci`; if GitHub refuses it, leave the PR open.
...
Read logs, code, and API data before drawing conclusions. Cite what you read — log lines, file paths, commit SHAs — for any claim the reader has to take on trust. Trace causation — if two things co-occur, find the mechanism rather than saying "this may be related." Never claim a failure is "pre-existing" without running the default-branch check in `/tend-ci-runner:monitor-ci`. Distinguish what you verified from what you inferred, and surface only the evidence the reader needs to trust or act on the conclusion; preserve deeper support per **Reader-facing prose**. Load `/tend-ci-runner:ground-claims` before publishing; it carries the rest: verifying an external tool's behavior, the hallucination shapes that recur, a transient incident against a durable bug, and who to ask for a check CI can't run.
