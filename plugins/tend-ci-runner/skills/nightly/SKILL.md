...
NON_STAMP_DIFF=$(git diff --no-color .github/workflows .config \
  | grep -E '^[+-]' \
  | grep -vE '^(\+\+\+|---) ' \
  | grep -vE '^[+-]# Generated by tend [0-9]+\.[0-9]+\.[0-9]+$' \
  | wc -l)
```
...
- **Version bumped**: add a `**tend version:** OLD → NEW` line, then a short **Notable changes** list — 3–5 bullets summarizing the user-relevant entries in `/tmp/tend-upstream-commits.txt`. Rewrite each `(#NNN)` ref as `max-sixty/tend#NNN` — a bare `#NNN` auto-links to this repo's own issues, not tend's. Skip pure mechanics (`chore: regenerate workflows`, `chore: release`, action-pin and lockfile bumps). Close with the compare link printed above. If the commits file is empty (the compare call failed), keep just the version line and the compare link.
