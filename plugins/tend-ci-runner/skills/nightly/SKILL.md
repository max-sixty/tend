...
permission, and where credentials live). Run the version the committed
workflows were generated by, so a newer release's checks wait for the
regeneration PR; `@latest` covers a checkout with no stamped workflows:

```bash
PINNED=$(uv run --script \
  "${CLAUDE_PLUGIN_ROOT}/scripts/nightly_workflow_update.py" version)
uv tool run "tend@${PINNED:-latest}" check 2>&1 | tee "$TMPDIR/tend-check.txt"
```
