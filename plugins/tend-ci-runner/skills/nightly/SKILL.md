...
# Migrate a legacy `.config/tend.toml` to `.config/tend.yaml` if present.
# `tend migrate` is a one-shot upgrade for adopters bumping past the
# TOML→YAML cutover — it verifies the parsed structures match before
# replacing the file, and is a no-op when the YAML is already in place.
if [ -f .config/tend.toml ] && [ ! -f .config/tend.yaml ]; then
  uvx tend@latest migrate
fi

uvx tend@latest init
# .config/ is checked alongside .github/workflows so a one-shot `tend migrate`
# (TOML→YAML rename + content swap) shows up in the same nightly PR as the
# regenerated workflows that depend on it.
