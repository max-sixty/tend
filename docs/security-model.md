...
a floating ref. Release tags on `max-sixty/tend` are immutable: a tag ruleset
restricts `update` and `deletion` (leaving `creation` open so the release can
push a new `X.Y.Z`), with no bypass for write-access actors. A leaked bot
token or hijacked session on `max-sixty/tend` therefore cannot move a
published tag and retroactively change the code every adopter already runs;
the worst it can do is push a new release tag, which adopters only pick up on
their next nightly regen, as a reviewable workflow-file diff in their own
repo. Adopters extend trust to `max-sixty/tend`'s release-tag integrity the
